...
const NewChatDialog = ({ onChatCreated, onOpenChange }: NewChatDialogProps) => {
  ...
  const { data, isFetching, isError, isSuccess } = useQuery({
    queryKey: ["streams-user", searchInputDebounce],
    queryFn: async () =>
      client.queryUsers(
        {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const chanel = client.channel("messaging", {
...
const UserResult = ({ user, selected, onClick }: UserResultProps) => {
  return (
    ...
    >
      ...
      {selected && <Check className="size-5 text-purple-500" />}
    </button>
  );
};
...
const SelectedUserTag = ({ onRemove, user }: SelectedUserTagProps) => {
  return (
    <button
      onClick={onRemove}
